...
namespace PeanutClub.Dealer.API
{
    ...
    public class DealerAudio
    {
        ...
        public void PlayRandomClip(ClipType type)
        {
            if (isPlaying)
            {
                ApiLog.Debug("DealerAudio", $"Already playing a clip, skipping ({type}) ..");
                return;
            }
            ...
            {
                ApiLog.Debug("DealerAudio", $"Clip {type} is in cooldown ({(Time.realtimeSinceStartup - clipTime)}s / {clipCooldown}s)");
                return;
            }
            ...
            {
                ApiLog.Debug("DealerAudio", $"No clips loaded ({type})");
                return;
            }
            ...
            isPlaying = PlaybackUtils.PlayAt(clipPath, Dealer.Player.Position, null, () =>
            {
                ...
                ApiLog.Debug("DealerAudio", $"Playback finished ({type})");
            });

            ApiLog.Debug("DealerAudio", $"Playback started ({type}): {isPlaying}");
        }
